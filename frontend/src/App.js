...
// URL 경로(pathname)와 mainTab 값을 서로 매핑합니다.
// 예: "/" -> "home", "/news" -> "news"
const PATH_TO_TAB = {
  "/": "home",
  "/news": "news",
...
function App() {
  ...
  // mainTab은 더 이상 useState 초기값으로 관리하지 않고, 현재 URL(pathname)에서 바로 계산합니다.
  // 새로고침을 해도 location.pathname이 그대로 유지되므로 mainTab도 항상 올바르게 복원됩니다.
  const mainTab = PATH_TO_TAB[location.pathname] || "home";

  // subTab(해외여행/국내축제)은 쿼리스트링(?sub=domestic)으로 관리합니다.
  ...
  // 탭 이동 헬퍼: 기존 코드에서 setMainTab("xxx") 형태로 호출하던 부분을
  ...
  useEffect(() => {
    // 1. 먼저 로컬 스토리지에 자체 백엔드 로그인 유저 정보가 있는지 체크합니다.
    ...
    if (localToken && localUser) {
      ...
      // 2. 없다면 기존 Supabase 세션 정보 확인 (하이브리드 유지)
  ...
  return (
    <div style={{ backgroundColor: "#f8fafc", minHeight: "100vh", paddingBottom: "60px" }}>
      ...
      <main style={{
        ...
        {/* 2. [우측] 메인 컴포넌트 렌더링 영역 */}
        {/* mainTab === "xxx" && <Component /> 방식 대신 <Routes>/<Route>를 사용합니다.
